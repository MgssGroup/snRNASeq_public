...
print("Set input and output directories")
input_dir <-
  "/home/malosree/projects/def-gturecki/malosree/Finalized_cluster_QC_annotation/"
output_dir <-
  "/home/malosree/projects/def-cnagy/malosree/Permutation_outputs/4_female_broad/"

print("Load data")
harmonized_object <- readRDS(file = paste0(input_dir, "1_enhanced_harmonized_object.Rds"))

print("Subset data")
MC_raw_clustered_Female <- subset(harmonized_object, Sex == "Female")

rm(harmonized_object)

original_case_control <- unique.data.frame(MC_raw_clustered_Female@meta.data[c("Sample",
...
#for(i in 1:3) {
	...
	permuted_case_control %>% group_by(Batch) %>% group_map(~permute_labels(i, .x, .y)) %>% bind_rows() -> permuted_case_control
        ...
        for (sample in permuted_case_control$Sample) {
                        MC_raw_clustered_Female@meta.data$Condition[MC_raw_clustered_Female@meta.data$Sample == sample] <-
                                   rep(permuted_case_control$Condition[permuted_case_control$Sample == sample],
                                        sum(MC_raw_clustered_Female@meta.data$Sample == sample))
        }

        #print(unique.data.frame(MC_raw_clustered_Female@meta.data[c("Sample",
        #                   "Condition")]))
	
	print(paste("Make and preprocess SingleCellExperiment object", i))
        sce <- as.SingleCellExperiment(MC_raw_clustered_Female)
	
	rm(MC_raw_clustered_Female)
	...
	print(paste("Differential expression", i))
	#pbDS####
