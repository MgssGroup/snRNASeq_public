...
output_dir <-
  "/home/malosree/projects/def-cnagy/malosree/Permutation_outputs/1_male_subtype/"
...
MC_raw_clustered_Male <-
  subset(harmonized_object, subset = (Sex == "Male" &  Sample != c("M24_2")))
...
#for(i in 1:3) {
	...
	temp <- colData(sce)
	rownames(temp) <- NULL
	anno <- unique.data.frame(temp[c("sample_id",
                           "group_id",
                           "Sex",
                           "Batch",
                           "Age",
	...
	contrast <-
		makeContrasts(Case_vs_Control = (group_idCase - group_idControl),
